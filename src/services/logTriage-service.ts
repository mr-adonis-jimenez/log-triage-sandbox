...
export class LogTriageService {
  ...
  getLogs(filter?: TriageFilter): LogEntry[] {
    ...
    if (filter.levels?.length) logs = logs.filter(log => filter.levels!.includes(log.level));
    if (filter.status?.length) logs = logs.filter(log => filter.status!.includes(log.triageStatus));
    if (filter.sources?.length) logs = logs.filter(log => filter.sources!.includes(log.source));
    if (filter.dateRange) {
